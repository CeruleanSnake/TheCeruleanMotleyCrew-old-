(function() {
	console.log("This website was programmed by Grant Gryczan.\n(But do note that he is not proud of the code.)");
	var magic = {};
	magic.magic = magic;
	console.log(magic);
	var idtoken;
							...
							if(statusType == 2) {
								...
								switch(req.status) {
									...
									case 404:
										...
										break;
									case 429:
										MSPFA.dialog("Error", document.createTextNode("Your client is sending data to MSPFA too quickly. Wait a moment before continuing."), ["Okay"]);
										break;
								}
	...
	var fetchDate = function(d, time) {
		return String(d).split(" ").slice(1, time ? 5 : 4).join(" ");
	};
	...
	var initPromptOptOut = function(evt) {
		evt.preventDefault();
		MSPFA.dialog("Opt out of ad", document.createTextNode("To opt out of this ad, you need to uncheck the box for it in your account's settings."), ["Go to settings", "Cancel"], promptOptOut);
	};
	var um3 = document.querySelector("#um3");
	if(um3) {
		um3.parentNode.querySelector(".optOut").addEventListener("click", initPromptOptOut);
	...
	var getStoryCell = function(story) {
		...
		td2.appendChild(document.createTextNode(localStorage.favText === undefined ? story.f.length+story.g.length : localStorage.favText));
		td2.appendChild(document.createElement("br"));
		if(story.t.length) {
	...
	var login = function(d) {
		if(!d) {
			...
			if(d.f) {
				...
				MSPFA.dialog("Login", document.createTextNode("Are you at least 13 years old?"), ["Yes", "No"], function(output) {
					if(output == "Yes") {
						finalStep();
					} else if(output == "No") {
						setTimeout(function() {
							MSPFA.dialog("Login", document.createTextNode("You must be at least 13 years of age to sign up."), ["Okay"]);
						});
					}
				});
			} else {
			...
			*/
			...
			if(location.pathname == "/" && params.s != undefined) {
				...
				MSPFA.request(0, {
					...
					if(s.length) {
						...
						for(var i = 0; i < s.length; i++) {
							...
							var img = new Image();
							img.classList.add("cellicon");
							img.src = s[i].o || "/images/wat/random.njs?cb=" + (s[i].i % 16);
							td1.appendChild(img);
							tr.appendChild(td1);
							var td2 = document.createElement("td");
							var t = document.createElement("a");
				...
				document.querySelector("#bannertip").addEventListener("click", function() {
					MSPFA.dialog("Info", document.createTextNode("This image should be 940x90 and will be displayed on the top of the homepage for one week on and after the anniversary of your adventure if the adventure is ongoing or complete and has 200 favorites or more."), ["Okay"]);
				});
				...
				if(params.s == "new") {
					...
					MSPFA.request(0, {
						...
						if(story.l == 2 && !MSPFA.me.p) {
							...
							if(MSPFA.me.i == "105279526507939039078" || MSPFA.me.i == "113383432424492821380" || MSPFA.me.i == "101485544748229804475") {
				...
				var changeAd = function(evt) {
					if(this.checked) {
						...
						MSPFA.dialog(":(", document.createTextNode("Ads are how we earn money, how we fund the website. By disabling this ad you are effectively decreasing our funds. We're okay with that if you don't think it's worth the inconvenient visual experience, but it would be highly appreciated if you first considered the effect of your choice."), ["I understand the impact of my decision and would like to proceed.", "Cancel"], function(output) {
							if(output != "Cancel") {
					...
					if(m.s.length) {
						...
						for(var i = 0; i < m.s.length; i++) {
							(function(msi) {
								...
								var img = new Image();
								img.classList.add("cellicon");
								img.src = m.u[msi.f].o || "/images/wat/random.njs?cb=" + msi.i % 16;
								td1.appendChild(img);
								tr.appendChild(td1);
								var td2 = document.createElement("td");
								td2.style.width = "100%";
	...
	if(document.cookie) {
		try {
			...
			if (window.gapi) {
				...
				const failsafeGAPICheck = setInterval(function () {
					if (window.gapi) {
						clearInterval(failsafeGAPICheck);
						gapiLoad();
					}
				}, 250);
			}
	...
	if(location.pathname == "/" && params.s == undefined) {
		...
		var getStoryTile = function(story, status) {
			...
			s.appendChild(document.createTextNode(localStorage.favText === undefined ? story.f.length+story.g.length : localStorage.favText));
			return s;
		};
		...
		MSPFA.slinkSlide = function(parent) {
			const as = parent.querySelectorAll("a[href]:not([target=_blank]), area[href]:not([target=_blank])");
		...
		MSPFA.request(0, {
			...
			if(location.pathname != "/preview/") {
				...
				td2.appendChild(heartIcon.cloneNode(false));
				td2.appendChild(document.createTextNode(localStorage.favText === undefined ? MSPFA.story.f.length+MSPFA.story.g.length : localStorage.favText));
				td2.appendChild(document.createElement("br"));
					...
					loadComments = function(load) {
						...
						MSPFA.request(0, req, function(c) {
							...
							for(var i = 0; i < c.c.length; i++) {
								...
								var img = new Image();
								img.classList.add("cellicon");
								img.src = c.u[c.c[i].u].o || "/images/wat/random.njs?cb=" + c.c[i].d % 16;
			...
			if(MSPFA.story.t.indexOf("nsfw") !== -1 || MSPFA.story.t.indexOf("translation") !== -1 || MSPFA.story.t.indexOf("mirror") !== -1) {
				var um3container = document.querySelector("#um3container");
				if(um3container) {
					um3container.parentNode.removeChild(um3container);
				}
		...
		MSPFA.request(0, {
			...
			MSPFA.request(0, {
				...
				for(var i = s.length-1; i >= 0; i--) {
					if (!s[i].p || s[i].t.indexOf('test') !== -1 || s[i].t.indexOf('unlisted') !== -1) {
						s.splice(i, 1);
					}
				}
				if(s.length) {
		...
		MSPFA.request(0, {
			...
			if(!story.l && story.p.length) {
				...
				for(var i = MSPFA.story.p.length-1; i >= 0; i--) {
					...
					pages.appendChild(document.createTextNode(fetchDate(new Date(MSPFA.story.p[i].d)) + " - "));
					var cmdlink = document.createElement("a");
					cmdlink.href = "/?s=" + MSPFA.story.i + "&p=" + (i+1);
					cmdlink.appendChild(document.createTextNode("\""));
					cmdlink.appendChild(MSPFA.parseBBCode(MSPFA.story.p[i].c || MSPFA.story.m));
		...
		vote.addEventListener("submit", function(evt) {
			...
			if(idtoken) {
				MSPFA.request(0, {
					...
					if(story.l) {
					...
					} else if(story.e.indexOf(MSPFA.me.i) != -1 && story.t.indexOf("mirror") == -1) {
						MSPFA.dialog("Error", document.createTextNode("You author that adventure."), ["Okay"]);
						...
						MSPFA.dialog("Error", document.createTextNode("Shitposts cannot be featured, as the homepage of a community often represents its community, and to place a shitpost in that position would be a very inaccurate representation."), ["Okay"]);
					} else if(story.t.indexOf("nsfw") != -1) {
						MSPFA.dialog("Error", document.createTextNode("NSFW content cannot be featured."), ["Okay"]);
					} else {
